...
export async function loadChat() {
    ...
    supabase
        ...
        .on('postgres_changes', { 
            ...
            // Only append if it's not one we just sent (deduplication check could be here, but simpler to just append)
            // Actually, if we optimistic update, we might duplicate. 
            // Let's check if we already have this message ID or similar content/timestamp? 
...
async function syncPendingMessages() {
    const pending = JSON.parse(localStorage.getItem('chat_pending_messages') || '[]');
    ...
    localStorage.setItem('chat_pending_messages', JSON.stringify(newPending));
    if (pending.length > newPending.length) {
        showToast('Offline messages synced successfully ✅', 'success');
    }
}
...
export async function sendComment() {
    ...
    const ipAddress = await getIP().catch(() => 'unknown');
    const msgObj = {
        user_name: name,
        message: message,
        ip_address: ipAddress,
        created_at: new Date().toISOString(),
        timestamp: Date.now() // For local tracking
    };
    ...
    const pending = JSON.parse(localStorage.getItem('chat_pending_messages') || '[]');
    pending.push(msgObj);
    localStorage.setItem('chat_pending_messages', JSON.stringify(pending));
    ...
    try {
        ...
        if (error) {
            ...
            if (error.message.includes('banned')) {
                showToast('මෙම පරිශීලකයා තහනම් කර ඇත', 'error');
                // Remove from local if banned
                 const currentPending = JSON.parse(localStorage.getItem('chat_pending_messages') || '[]');
                 const filtered = currentPending.filter(m => m.timestamp !== msgObj.timestamp);
                 localStorage.setItem('chat_pending_messages', JSON.stringify(filtered));
            } else {
                 // Keep in local storage (already there)
                 showToast('Connection lost. Message saved and will retry. 📡', 'info');
            }
            return;
        }
        
        // 4. Success - Remove from local
        const currentPending = JSON.parse(localStorage.getItem('chat_pending_messages') || '[]');
        const filtered = currentPending.filter(m => m.timestamp !== msgObj.timestamp);
        localStorage.setItem('chat_pending_messages', JSON.stringify(filtered));
        ...
        showToast('පණිවිඩය යැවීමේ දෝෂයක්. පසුව නැවත උත්සාහ කරනු ලැබේ.', 'warning');
