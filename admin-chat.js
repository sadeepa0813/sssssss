        ...
        if (allChatMessages.length > 0) {
            allChatMessages.forEach(comment => {
                ...
                row.innerHTML = `
                    ...
                    <td data-label="Message">${sanitizeHTML(comment.message.length > 100 ? comment.message.substring(0, 100) + '...' : comment.message)}</td>
                    <td data-label="IP Address">${sanitizeHTML(comment.ip_address || 'N/A')}</td>
                    <td data-label="Time">${date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</td>
