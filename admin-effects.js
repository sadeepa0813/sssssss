...
export async function loadEffectsStatus() {
    try {
        const { data, error } = await supabase
            .from('site_settings')
            .select('*');
            //.in('setting_key', ['snow_effect', 'confetti_effect', 'dark_theme']); // Load all settings
            ...
            data.forEach(setting => {
                ...
                if (element) {
                    if (element.type === 'checkbox') {
                        element.checked = setting.is_enabled;
                    } else {
...
export async function saveSettings(category) {
    showLoading(true);
    try {
        let settingsToSave = [];
        ...
        if (category === 'security') {
            ...
            settingsToSave.push({ key: 'sessionTimeout', value: timeout, enabled: true });
            settingsToSave.push({ key: 'twoFactorAuth', value: twoFactor ? 'true' : 'false', enabled: twoFactor });
        } else if (category === 'database') {
            const frequency = document.getElementById('backupFrequency').value;
            const autoBackup = document.getElementById('autoBackup').checked;
            
            settingsToSave.push({ key: 'backupFrequency', value: frequency, enabled: true });
            settingsToSave.push({ key: 'autoBackup', value: autoBackup ? 'true' : 'false', enabled: autoBackup });
            ...
            settingsToSave.push({ key: 'emailNotifications', value: email ? 'true' : 'false', enabled: email });
            settingsToSave.push({ key: 'pushNotifications', value: push ? 'true' : 'false', enabled: push });
            settingsToSave.push({ key: 'notificationSound', value: sound, enabled: true });
        }
